...
function fastestResponseTime() {
    ...
    let maxLastResponseTime = server.responseTime[server.responseTime.length - 1]
    for (let i = 1; i < servers.length; i++) {
        let currentLastRespTime = servers[i].responseTime[servers[i].responseTime.length-1]
        if (currentLastRespTime > maxLastResponseTime) {
            maxLastResponseTime = currentLastRespTime;
            server = servers[i]
        }
    }

    return {name: server.name, time: maxLastResponseTime};
}

console.log(servers)
for (let i = 0; i < 10; i++) {
    const selectedServer = fastestResponseTime();
    console.log(` \nSlowest Response Time: ${selectedServer.name} with last response time ${selectedServer.time} ms`);
